...
- **仅处理私聊**：即使 Bot 被加入群组并设为管理员，也会忽略群组、超级群组和频道消息，不会转发群消息或要求群成员进行人机验证
...
- 连续验证失败两次后暂停验证24小时，期间不能重新发起验证
- 随机使用三数加法、减法、两位数乘法、混合运算、数列规律和第二大数字等验证题，并提供8个乱序近似选项
...
- 🛠️ 修复验证有效期误判：验证成功时间现在会作为有效活动时间，避免用户刚通过验证就因历史消息过旧而被重复要求验证。
- 🛠️ Python 服务器版和 Cloudflare Worker 版均同步修复，验证成功后会清理旧验证题状态。
- 🛡️ 修复 Bot 加入群组并成为管理员后，错误识别和转发群消息的问题。
- 🛡️ 修复群成员被错误要求完成人机验证的问题。
- Python 服务器版的命令、管理员回复和用户中继均限制为私聊。
- Cloudflare Worker 版会直接忽略群组、超级群组及频道消息。
- 私聊双向中继、验证、会话管理等原有功能保持不变。
- 增加验证防爆破：连续失败两次后锁定24小时，验证成功后失败计数清零。
- 🔐 强化人机验证难度：使用三数加法、减法、一位数乘法、简单混合运算、数列规律、第二大数字共 6 种随机题型；不使用两位数乘法。
- 🔐 验证选项由 4 个增加至 8 个，改为接近正确答案的乱序干扰项，降低随机点击和简单脚本通过的概率。
- ⏱️ 每道验证题有效期为 2 分钟，超过 2 分钟未完成按验证失败处理，并计入失败次数。
...
如果这个项目对你有帮助，欢迎在 GitHub 点一个 ⭐ **Star**。你的支持会帮助更多人发现这个项目，也会鼓励项目持续维护和改进！
